...
import com.pro.exp.model.request.SysEmployeeForm;
...
/**
 * Description: pro-exp
 * Created by JT on 2018/11/25 22:09
 */
@RestController
@Api(value = "人员管理",tags = "人力管理-人员管理" ,description = "人员管理")
...
public class SysEmployeeController extends BaseController {
    ...
    @ApiOperation(value = "增加基础人员")
    @ApiImplicitParam(name = "pr-token", value = "登录凭证", required = true, paramType = "header")
    @PostMapping("/")
    @InsertAction
    public ObjectResponse<IDResponse> addRole(@RequestBody SysEmployeeForm form,
    ...
    @ApiOperation(value = "获取基础人员")
    @ApiImplicitParam(name = "pr-token", value = "登录凭证", required = true, paramType = "header")
    @GetMapping("/{id}")
    @SelectAction
    public ObjectResponse<SysEmployeeResponse> getSysRole(@PathVariable("id") String id) {
        SysEmployee bean = service.selectByPrimaryKey(id);
        SysEmployeeResponse beanResponse = new SysEmployeeResponse();
        if (bean == null) {
    ...
    @ApiOperation(value = "更新基础人员")
    @ApiImplicitParam(name = "pr-token", value = "登录凭证", required = true, paramType = "header")
    @PutMapping("/{id}")
    @UpdateAction
    public ObjectResponse<IDResponse> updateSysRole(@PathVariable("id") String id,
                                                    @RequestBody SysEmployeeForm form,
                                                    HttpServletRequest request, HttpServletResponse response) {
        SysEmployee bean = new SysEmployee();
        checkParamNotNull(form);
        BeanUtils.copyProperties(form, bean);
        bean.setSysUpdatedby(getEmployeeInfo(request).getSysEmployeeId());
        bean.setSysEmployeeId(id);
        service.updateByPrimaryKey(bean);
        response.setStatus(HttpStatus.CREATED);
        return response(new IDResponse(id));
    }

    @ApiOperation(value = "条件查询基础人员")
    @ApiImplicitParam(name = "pr-token", value = "登录凭证", required = true, paramType = "header")
    @PostMapping("/query")
    @SelectAction
    public PagedResponse<SysEmployeeResponse> querySysRole(@RequestBody BaseSelectForm form, HttpServletResponse response) {
        form.setClassName("SysEmployee");
        PageInfo<Map<String, Object>> list = baseExampleService.selectByPrimary(form);
        PageInfo<SysEmployeeResponse> pageInfo = new PageInfo<>();
